...
REV=7
bpy.ops.wm.open_mainfile(filepath=os.path.join(OUT,'soldier_medium_body.blend'))
...
tan=mat('helmet fabric tan',(.34,.28,.19));brown=mat('carrier coyote brown',(.12,.088,.05));olive=mat('olive webbing',(.31,.34,.205));dark=mat('charcoal fabric',(.025,.028,.023));shield=mat('graphite knee armor',(.075,.085,.08));glass=mat('black smoked glass',(.006,.010,.008))
...
def pouch(name,center,size,m):
 x,y,z=center;w,d,h=size;vs=[];fs=[];n=12
 for zz,s in [(-.5,.73),(-.39,.95),(-.17,1.02),(.18,1.0),(.38,.92),(.5,.68)]:
  for j in range(n):
   a=math.tau*(j+.5)/n;c=math.cos(a);q=math.sin(a)
   vs.append((x+math.copysign(abs(c)**.48,c)*w*.5*s,y+math.copysign(abs(q)**.48,q)*d*.5*s,z+zz*h))
 for r in range(5):
  for j in range(n):
   a=r*n+j;b=r*n+(j+1)%n
   fs.extend([(a,b,b+n),(a,b+n,a+n)])
 fs.extend([tuple(reversed(range(n))),tuple(5*n+j for j in range(n))]);return mesh(name,vs,fs,m)
...
carrier=shell('Equipment_plate_carrier',[(1.065,.166,.145,.135),(1.105,.18,.162,.14),(1.20,.187,.175,.15),(1.31,.201,.178,.15),(1.365,.198,.17,.14),(1.415,.168,.145,.12)],brown)
carrier.data.materials.append(tan)
for p in carrier.data.polygons:
 if p.center.z>1.34:p.material_index=1
...
for side in [-1,1]:
 ...
 pouch('Equipment_side_pouch',(side*.22,0,1.18),(.088,.17,.18),olive)
# Substantial pack depth is taken from the side reference.
pouch('Equipment_backpack',(0,-.196,1.295),(.32,.185,.415),olive)
...
verts=[];faces=[];n=16
for z,rx,ry in [(1.655,.123,.13),(1.695,.129,.135),(1.735,.115,.125),(1.77,.090,.103),(1.795,.054,.065),(1.802,.015,.021)]:
...
faces.append(tuple(5*n+j for j in range(n)));mesh('Equipment_helmet',verts,faces,tan)
...
# Deliberate faceted textile tones avoid smooth cylinders while retaining silhouette.
import random
rng=random.Random(23)
for o in list(bpy.context.scene.objects):
 if o.type!='MESH':continue
 if o==body or any(s in o.name for s in ['pouch','backpack','helmet']):
  original=o.data.materials[0]
  base=original.diffuse_color[:3]
  for factor in [.86,.94,1.05,1.12]:
   o.data.materials.append(mat(original.name+' facet',tuple(v*factor for v in base)))
  for p in o.data.polygons:
   if rng.random()<.42:p.material_index=rng.randrange(len(o.data.materials))
# Continuous elbow cuffs emphasize natural taper in each sleeve.
for side in [-1,1]:
 vs=[];fs=[]
 for x in [.478,.505]:
  for j in range(12):
   a=j*math.tau/12;vs.append((side*x,.081*math.cos(a),1.40+.066*math.sin(a)))
 for j in range(12):fs.append((j,(j+1)%12,(j+1)%12+12,j+12))
 mesh('Equipment_elbow_cloth',vs,fs,brown)
...
for label,direction in {'front':(0,12,0),'side':(-12,0,0),'back':(0,-12,0),'top':(0,0,12),'top_3quarter':(-8,14,3)}.items():
 ...
 if label in ['front','side','top_3quarter']:
  ...
  scene.render.film_transparent=True;scene.render.filepath=os.path.join(OUT,f'soldier_medium_r{REV}_'+label+'_alpha.png');bpy.ops.render.render(write_still=True);scene.render.film_transparent=False
triangles=0
for o in scene.objects:
 if o.type=='MESH':o.data.calc_loop_triangles();triangles+=len(o.data.loop_triangles)
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT,'soldier_medium_equipment_base.blend'))
with open(os.path.join(OUT,'soldier_medium_equipped_stats.json'),'w') as f:json.dump({'triangles':triangles,'body_unchanged':digest()==before,'body_vertex_hash':before,'rigged':False,'view_bounds':view_bounds,'inputs':['soldier_medium_body.blend','three generated turnaround images']},f,indent=2)
