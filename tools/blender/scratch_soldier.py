...
torso=[(.80,.115,.105),(.88,.185,.125),(.99,.18,.12),(1.09,.185,.125),(1.20,.20,.14),(1.35,.21,.135),(1.43,.17,.11),(1.49,.075,.075)]
...
for side in [-1,1]:
 ...
 rs=[(.070,.083),(.075,.086),(.071,.080),(.065,.074),(.055,.061),(.065,.070),(.067,.069),(.060,.061),(.044,.046)]
 ...
 radii=[(.070,.085),(.100,.11),(.100,.106),(.092,.09),(.081,.082),(.074,.075),(.085,.09),(.075,.077),(.057,.06),(.055,.058)]
...
bpy.ops.object.quadriflow_remesh(target_faces=3000,use_mesh_symmetry=True)
...
for label,direction in {'front':(0,12,0),'side':(12,0,0),'back':(0,-12,0),'top':(0,0,12),'top_3quarter':(8,12,9)}.items():
 camera.location=center+Vector(direction);camera.rotation_euler=(center-camera.location).to_track_quat('-Z','Y').to_euler();scene.render.filepath=os.path.join(OUT,f'soldier_scratch_r{REV}_'+label+'.png');bpy.ops.render.render(write_still=True)
 if label=='front':
