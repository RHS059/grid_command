...
test('aircraft have distinct silhouettes and independently animated parts', () => {
  ...
  for (const name of ['main-rotor', 'tail-rotor', 'tandem-canopy', 'chin-gun']) assert.ok(heli.getObjectByName(name), name)
  for (const name of ['nose-cone', 'canopy', 'tail-fin']) assert.ok(jet.getObjectByName(name), name)
  ...
  assert.equal(heli.getObjectByName('main-rotor')!.rotation.z, 70)
  assert.equal(heli.getObjectByName('tail-rotor')!.rotation.x, 96)
  assert.equal(uav.getObjectByName('propeller')!.rotation.y, 90)
  const before = heli.getObjectByName('main-rotor')!.rotation.z; animateAircraft(heli, 2); assert.equal(heli.getObjectByName('main-rotor')!.rotation.z, before)
