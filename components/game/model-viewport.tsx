...
interface Props { model: ModelId; side: Side; active: boolean; animate: boolean; rotate: boolean; action: SoldierAction; stance: Stance; condition: Soldier['status']; destroyed?: boolean; reset: number; clip?:string; loop?:boolean; seek?:{serial:number;time:number}; onTime?:(time:number)=>void }
export function ModelViewport(props: Props) {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      if (object) object.userData.destroyed = !!c.destroyed
