...
# 48. CHANGELOG — GRID COMMAND V2

The following entries document the implemented changes added after the V37 handoff baseline. Each item is intentionally separate so a future developer or AI can audit the current behavior.

- **Version 0.3 baseline:** Rebuilt the browser implementation as a Next.js 16 / React 19 application with a real San Diego OpenFreeMap geographic map, MapLibre 4.7.1, AWS Terrarium DEM terrain, real building extrusions and water, and a Three.js battlefield overlay.
- **Navigation and geometry:** Added bounded geometry preloading, terrain/building visibility rays, courtyard holes, first-hit obstruction, explicit geometry readiness, cached geometry reuse on restart, 25 m navigation cells, dry/passable placement safeguards, and terrain-grounded model placement.
- **Combat and weapons:** Added rifle/MG engagement limits, distance-dependent spread, shooter-specific visibility and muzzle clearance, mortar sight/arc checks, weapon-specific armor damage rules, authoritative projectile events, explosive effects, smoke, cover, and casualty/rescue lifecycle limits.
- **Units and models:** Added articulated low-poly infantry with aiming, stance, recoil, equipment silhouettes, casualty poses, role-specific equipment, detailed supply trucks, distinct tank/APC/IFV/helicopter/jet/UAV factories, BLU/RED liveries, animated aircraft parts, landing gear, rotors and UAV propellers.
- **Base compounds:** Added visible BLU and RED MOB and airfield compounds with headquarters, communications, barriers, supplies, runways, taxiways, hangars, control towers, helipads and fuel tanks; added raised terrain-aligned foundations and excluded overlapping map-building visuals.
- **Objectives and match rules:** Made A–E objectives neutral on initial load and restart, retained the six-active-body capture minimum, added objective control timing, commander-loss defeat, simultaneous territorial victory requirements, contested-hold resets, and removed the match time limit.
- **Commander planning:** Added deterministic local Web Worker planning, simultaneous commander submissions, strategic search and caching, imperfect-information contact reports, tempo pressure, high-level mission semantics, multiple assault groups, and executable local squad orders.
- **Force generation:** Changed the runtime to commander-only starts with 2,000 SP per side and empty depots; autonomous requisitions now prioritize infantry and transport before heavy assets and maintain a larger wartime force.
- **Physical logistics:** Added external scheduled cargo flights, airfield handling, forklifts, delivery trucks, MOB convoys, location-specific fuel/ammunition/repair inventory, manifest conservation, facility-specific supply transfers, empty vehicle arrivals, engine fuel consumption, ammunition requirements, repair-material consumption, and landed-aircraft/ground-vehicle servicing.
- **Camera and selection:** Added shared display poses, frame-driven rear chase cameras for infantry, vehicles and aircraft, carrier-following embarked selections, follow release from Overview/manual camera controls, stable close-up infantry framing, bounded terrain zoom, and corrected MapLibre target-relative elevation handling.
- **Airfield upgrade doctrine:** Added tiered airfields. Tier 1 has one landing strip, one forklift and one delivery truck. Tier 2 has one strip, two forklifts and two delivery trucks. Tier 3 has two strips, four forklifts and four delivery trucks with two trailers each. Supply throughput and SP income scale 1×/2×/4×; commanders prioritize upgrades after initial infantry and transport, with tier 2 costing 600 SP/60 seconds and tier 3 costing 1,200 SP/90 seconds.
- **Airfield capacity rendering:** Added a second landing strip, independent runway slots, concurrent tier 3 cargo flights, tier-scaled cargo manifests, visible forklifts, delivery trucks, articulated double trailers, tier-aware airfield geometry, and logistics-panel capacity/cost/countdown disclosure.
- **Transport helicopter capacity:** Increased transport helicopters to 24 troop seats, excluding crew, so six four-person rifle squads can be carried.
- **Transport helicopter assault gate:** Required an assault helicopter to have at least 12 active troops before dispatch; a hovering or partially loaded transport cannot begin a capture assault.
- **Transport helicopter unloading:** Required touchdown at zero altitude before unloading; passengers now disembark one soldier per simulation second, remain hidden aboard until disembarked, and cannot capture or contest while embarked.
- **Capture eligibility:** Restricted capture and contest contribution to active dismounted infantry. Helicopter crews, aircraft, vehicles, hovering transports, and embarked passengers do not count; capture begins only after the helicopter unloads its troops.
- **Fuel balance:** Doubled engine fuel consumption for vehicles and aircraft classes that consume fuel.
- **Refueling and rearming balance:** Divided refueling and rearming rates by three, including infantry depot rearming; with sufficient stock, empty-to-full service is approximately 60 seconds. Repair speed remains unchanged.
- **UI and documentation:** Added airfield tier status, income, equipment capacity, upgrade countdown, costs, current logistics doctrine, helicopter assault requirements, build identifier 0.6, release notes, and mobile-safe scrollable detail panels.
- **Validation:** Added regression coverage for autonomous tier upgrades, independent cargo landings, tier-scaled manifests, trailer capacity, sequential helicopter unloading, minimum assault load, capture exclusion, doubled fuel burn, slower refueling/rearming, neutral objectives, model coverage, camera math, and renderer disposal.
- **Release verification:** Final release checks recorded 80 passing tests, TypeScript validation, production build success, desktop and mobile browser inspection, natural tier 3 upgrades for both commanders, rendered second runway and tier logistics, opened capacity disclosure, and no mobile horizontal overflow. Software-rendered WebGL remains slow; physical-GPU performance and an end-to-end browser helicopter sortie are not certified.
