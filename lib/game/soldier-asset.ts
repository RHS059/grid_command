...
export function createSoldierTemplate(source:T.Object3D,side:Side){
  ...
  template.traverse(node=>{if(!(node instanceof T.Mesh))return;let geometry=geometries.get(node.geometry);if(!geometry){geometry=node.geometry.clone();geometries.set(node.geometry,geometry)}node.geometry=geometry
