...
export type SubcommanderId = 'TROOPS' | 'FUEL' | 'MOTORCADE' | 'AIR' | 'LOGISTICS' | 'FIRES'
export interface SubcommanderReport { id: SubcommanderId; name: string; responsibility: string; approved: boolean; required: boolean; status: string; reason: string; metrics: string; updatedAt: number }
export interface CommandCouncil { revision: number; action: StrategicAction; target: string; issuedAt: number; infantryApproved: boolean; airApproved: boolean; firesApproved: boolean; reports: Record<SubcommanderId, SubcommanderReport> }
