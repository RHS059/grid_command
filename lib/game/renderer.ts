import { EngineBoil, applyEngineBoil } from './engine-boil'
import { syncAircraftWeapons, createAircraftProjectile } from './aircraft-weapons'
...
import { airfieldPlatform, baseSurfaceElevation, createBase, groundBase, type BaseElevation } from './base-models'
...
import { BattlefieldVehicleBreakup } from './vehicle-breakup-renderer'

export class BattlefieldRenderer {
  private engineBoils = new Map<string,EngineBoil>()
  private aircraftProjectiles = new Map<number,T.Mesh>()
  ...
  aircraft=new Map<string,T.Group>();objectiveFacilities=new Map<string,T.Group>();bases:{model:T.Group;point:{x:number;y:number;id:string};elevation?:BaseElevation;terrainRevision?:number}[]=[];buildings:ModularBuildingRenderer
  ...
  readonly vehicleBreakup = new BattlefieldVehicleBreakup(this.scene)
  constructor(public map:GeographicMap,_canvas:HTMLCanvasElement|null,public getState:()=>BattleState,public getSettings:()=>{graphics:Graphics;perspective:Perspective;selected:string|null;active?:boolean},public onFPS:(n:number)=>void){
    ...
    for(const side of ['BLU','RED'] as Side[])for(const role of Object.keys(CATALOG) as Role[])for(const destroyed of [false,true]){if(!isVehicle(role)||isAir(role)||isSupportModel(role))continue;const suffix=destroyed?'-wreck':'';const mesh=new T.InstancedMesh(vehicleGeometry(role,side),material,64);mesh.count=0;mesh.frustumCulled=false;mesh.instanceMatrix.setUsage(T.DynamicDrawUsage);mesh.userData.destroyed=destroyed;mesh.userData.unitSurface=true;this.groups.set(`${side}-${role}${suffix}`,mesh);this.scene.add(mesh);if(['IFV','ATTACK_HELI'].includes(role)){const turret=new T.InstancedMesh(vehicleGeometry(role,side,true),material,64);turret.count=0;turret.frustumCulled=false;turret.userData.destroyed=destroyed;turret.userData.unitSurface=true;this.groups.set(`${side}-${role}${suffix}-attachment`,turret);this.scene.add(turret)}}
  ...
  render(matrix:number[]){if(this.disposed||this.getSettings().active===false){this.previous=0;return;}const now=performance.now(),state=this.getState(),{graphics,perspective,selected}=this.getSettings();if(this.previous)this.frames.push(now-this.previous);this.previous=now;if(now-this.report>1500&&this.frames.length){this.onFPS(Math.round(1000/(this.frames.reduce((a,b)=>a+b,0)/this.frames.length)));this.frames=[];this.report=now}
    ...
    this.vehicleBreakup.update(state,time,p=>this.altitude(p,now),c=>graphics.models&&(perspective==='OBS'||c.observed.includes(perspective))&&nearby(c.x,c.y,100),this.aircraft,graphics)
    for(const base of this.bases){
      ...
      groundBase(base,terrain,this.terrainRevision,bounds=>this.map.queryTerrainRange(bounds))
    ...
    const liveAircraft=new Set([...state.units.filter(u=>u.hp>0&&(isAir(u.role)||isSupportModel(u.role))).map(u=>u.id),...state.casualties.filter(c=>!this.vehicleBreakup.has(c.id)).map(c=>`wreck-${c.id}`)]);for(const[id,model]of this.aircraft){if(!liveAircraft.has(id)){this.scene.remove(model);disposeModel(model);this.aircraft.delete(id)}else model.visible=false}
    ...
    const visibleCasualties=!graphics.models?[]:state.casualties.filter(c=>!this.vehicleBreakup.has(c.id)&&(!performanceMode||zoom>12&&nearby(c.x,c.y))&&(perspective==='OBS'||c.observed.includes(perspective))).map(c=>({...c,id:`wreck-${c.id}`,hp:0,members:0,maxMembers:0,ammo:0,fuel:0,path:[],mission:'DESTROYED',target:'',name:'Wreck',kills:0,subcommand:'',firing:false,spotted:true,soldiers:c.soldier?[c.soldier]:[]} as Unit))
    const presentIds = new Set(state.units.map(u=>u.id))
    for(const id of this.engineBoils.keys())if(!presentIds.has(id))this.engineBoils.delete(id)
    ...
    for(const u of !graphics.models?[]:[...state.units.filter(u=>(!u.carrier||u.soldiers?.some(s=>s.disembarked))&&(u.hp>0||u.soldiers?.some(s=>s.status==='downed'))),...visibleCasualties]){if(perspective!=='OBS'&&u.side!==perspective&&!u.spotted)continue;const ll=lngLat(u),dist=Math.hypot((ll[0]-center.lng)*93650,(ll[1]-center.lat)*111320);if((dist>(performanceMode?1000:1700)||performanceMode&&zoom<=12)&&u.id!==selected)continue;const z=isNaval(u.role)&&u.role!=='AMPHIBIOUS_APC'?0:this.altitude(u,now)
      ...
      let boil=this.engineBoils.get(u.id);if(!boil){boil=new EngineBoil(u.role,u.id);this.engineBoils.set(u.id,boil)}
      const motorPose=boil.sample(time,!!u.engine,u.path.length>0,u.hp>0,u.role==='FORKLIFT'?3:8)
      if(isAir(u.role)||isSupportModel(u.role)){let model=this.aircraft.get(u.id);if(!model){model=isAir(u.role)?createAircraft(u.role,u.side):createSupportModel(u.role,u.side);if(u.role==='TROOP_TRUCK')addCarrierOccupants(model,u.side);
        ...
        model.traverse(o=>{if(o instanceof T.Mesh){for(const m of Array.isArray(o.material)?o.material:[o.material])if(m instanceof T.MeshStandardMaterial){m.flatShading=false;m.needsUpdate=true}}});this.aircraft.set(u.id,model);this.scene.add(model)}const damage=Math.max(0,Math.min(1,1-u.hp/100));model.userData.destroyed=damage>0;model.userData.vehicleDamage=damage>0?{damage,destruction:u.hp<=0?.42:0,heat:u.hp<=0?.32:0}:undefined;model.visible=graphics.models;model.position.set(u.x,u.y,z+(u.altitude||0)+.1);model.rotation.set(u.hp<=0?.18:0,(u.role==='JET'||u.role==='CAS_FIGHTER')?Math.sin(time)*.06:0,-u.heading);animateAircraft(model,u.hp>0&&u.engine?time:0,{x:u.x,y:u.y,heading:u.heading,aim:u.aim,time:state.time,active:u.hp>0&&!!u.engine});syncAircraftWeapons(model,u.aircraftLoadout);animateSupport(model,time,u.transport||{phase:'waiting',since:state.time,containerState:'empty'});if(u.role==='TROOP_TRUCK')updateCarrierOccupants(model,u,state);const chin=model.getObjectByName('chin-turret');if(chin)chin.rotation.z=u.heading-(u.aim??u.heading);const sling=model.getObjectByName('sling-cargo');if(sling)sling.visible=!!u.transport?.cargo&&(u.altitude||0)>8;const ramp=model.getObjectByName('cargo-ramp');if(ramp)ramp.rotation.x=u.transport?.phase==='unloading'?1.2:0;if(u.transport?.phase==='departed'||u.transport?.phase==='waiting'&&u.role==='CARGO_PLANE')model.visible=false;applyEngineBoil(model,motorPose)}
      else if(isVehicle(u.role)){const key=`${u.side}-${u.role}${u.hp<=0?'-wreck':''}`,mesh=this.groups.get(key)!,n=counts.get(key)||0;if(n>=64)continue;this.dummy.position.set(u.x,u.y,z+(u.altitude||0)+.1);this.dummy.rotation.set(u.hp<=0?.18:0,(u.role==='JET'||u.role==='CAS_FIGHTER')?Math.sin(time)*.06:0,-u.heading);this.dummy.scale.setScalar(1);applyEngineBoil(this.dummy,motorPose);this.dummy.updateMatrix();mesh.setMatrixAt(n,this.dummy.matrix);mesh.setColorAt(n,new T.Color(1,1,1));if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;counts.set(key,n+1);const attachment=this.groups.get(`${key}-attachment`);if(attachment){this.dummy.quaternion.multiply(new T.Quaternion().setFromAxisAngle(new T.Vector3(0,0,1),u.role==='ATTACK_HELI'?time*35+u.heading:u.heading-(u.aim??u.heading)));this.dummy.updateMatrix();attachment.setMatrixAt(n,this.dummy.matrix);counts.set(`${key}-attachment`,n+1)}}
    ...
    const activeProjectiles = new Set<number>()
    for(const e of state.shots){if(perspective!=='OBS'&&e.side!==perspective&&!e.spotted)continue;const age=time-e.time;if(age<0)continue;const start=new T.Vector3(e.start.x,e.start.y,e.start.z),end=new T.Vector3(e.end.x,e.end.y,e.end.z);if(!graphics.terrain){start.z-=this.altitude({...e.start,id:`s${e.id}`},now);end.z-=this.altitude({...e.end,id:`e${e.id}`},now)}const length=start.distanceTo(end),duration=length/e.speed,p=Math.min(1,age/Math.max(.03,duration)),pos=start.clone().lerp(end,p),direction=end.clone().sub(start).normalize();if(e.weapon==='mortar')pos.z+=Math.sin(p*Math.PI)*length*.22
      const rocket=['at','aa','missile','rocket'].includes(e.weapon)
      if(e.weapon==='bomb'){pos.z=start.z+(end.z-start.z)*p*p;direction.set(end.x-start.x,end.y-start.y,2*p*(end.z-start.z)).normalize()}
      if(age<duration&&(e.weapon==='bomb'||e.weapon==='rocket')){let projectile=this.aircraftProjectiles.get(e.id);if(!projectile){projectile=createAircraftProjectile(this.aircraft.get(e.unit),e.weapon);if(projectile){this.aircraftProjectiles.set(e.id,projectile);this.scene.add(projectile)}}if(projectile){activeProjectiles.add(e.id);projectile.position.copy(pos);projectile.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),direction);projectile.visible=graphics.models&&effectVisible(pos.x,pos.y,pos.z,2)}}
      if(age<.11&&e.weapon!=='bomb'){const fade=1-age/.11,flash=start.clone().addScaledVector(direction,.35);place('muzzle',flash.x,flash.y,flash.z,.18+e.size*1.5,.18+e.size*1.5,(1+e.size*4)*fade,direction);illuminate(flash.x,flash.y,flash.z,55*fade,14+e.size*24)}
      if(age<duration){const size=Math.max(e.size,zoom<17?.18:.055),streak=rocket?1.1:Math.min(3.5,Math.max(.7,e.speed*.004));if(e.weapon!=='bomb'){place('tracer-core',pos.x,pos.y,pos.z,size,size,streak,direction);place('tracer-glow',pos.x,pos.y,pos.z,size*3.5,size*3.5,streak*1.25,direction);illuminate(pos.x,pos.y,pos.z,rocket?38:18,rocket?20:10)}
    ...
    for(const [id,projectile] of this.aircraftProjectiles)if(!activeProjectiles.has(id)){this.scene.remove(projectile);this.aircraftProjectiles.delete(id)}
