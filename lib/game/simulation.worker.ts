import { buildScenario } from './ai/scenario'
import { updateHierarchy } from './ai/hierarchy'
import { applyBehaviorMission, updateVehicleBehavior } from './ai/integration'
...
import { initialState, CATALOG, BASES, AIRBASES, type BattleState, type Side, type Unit, type Point, type Role, type OrderRefusalCode } from './types'
...
import { updateMaritime } from './maritime-controller'
import { isNaval } from './types'
...
import { commissioningFuelPercent, updateVehicleService, consumeFuel, syncDepotTotals } from './sustainment'
import { aiInfantry, InfantryDirector } from './infantry-ai'
...
function refuse(u: Unit, code: OrderRefusalCode, reason: string, retry = 60, target=u.target||'current order') {
  ...
  if(report)log(u.side,`${u.name}: Negative â€” cannot comply with those orders â€” ${reason}.`,'system')
...
function commanders() {
  if (state.scenario && !state.scenario.automaticReinforcements) return
  for (const side of sides) {
    const f = state.forces[side]
    const role = nextPurchase(state, side, deliveries.filter(d=>!d.unitId))
    if (role === 'AIRFIELD_UPGRADE') { if (startAirfieldUpgrade(state, side)) log(side, f.purchase, 'logistics') }
    else if (role === 'MOB_UPGRADE') { if(startMobUpgrade(state,side))log(side,f.purchase,'logistics') }
    else if (role) { const cost=requisitionCost(state,side,role),delay=requisitionDelay(state,side,role,Math.ceil((state.time+1)/60)*60+35-state.time); f.sp -= cost; f.queue++; f.purchase = `${role.replaceAll('_', ' ')} Â· ${cost} SP Â· ${Math.ceil(delay)}s`; deliveries.push({ side: side, role, due: state.time+delay }); log(side, `${role.replaceAll('_', ' ')} requisition approved. Assembly queued.`, 'logistics') }
  }
}
...
function combat() {
  ...
  // A surviving formation succeeds the command post; complete force loss still resolves the match.
  const defeated = sides.filter(side => state.units.some(u => u.side === side && u.role === 'COMMAND' && u.hp <= 0) && !state.units.some(u => u.side === side && u.hp > 0 && !u.surrendered && !u.crewBailed && !missionAsset(u.role) && !['PILOT', 'LOGISTICS'].includes(u.role)))
  if (defeated.length) state.winner = defeated.length === 2 ? 'DRAW' : defeated[0] === 'BLU' ? 'RED' : 'BLU'
}
function capture() {
...
function tick() {
  ...
  const revisions = sides.map(side => state.behavior?.sides[side].plan?.revision)
  for (const mission of updateHierarchy(state)) applyBehaviorMission(state, mission, route)
  for (const [index, side] of sides.entries()) {
    const plan = state.behavior?.sides[side].plan
    if (plan && plan.revision !== revisions[index]) log(side, `${plan.posture} objective ${plan.target.id}. ${plan.reasons.join(' ')}`, 'command')
  }
  if (state.tick % 600 === 1) commanders()
  if (state.tick % 10 === 1) updateVehicleBehavior(state, route)
  ...
  for (const u of state.units) {
    ...
    if (isNaval(u.role) || u.hp <= 0 || u.surrendered || u.crewBailed || u.servicing || u.emergency || u.deployment || u.external || u.carrier || u.attachedSquad || (troopSeats(u.role)>0&&u.transport&&u.transport.phase!=='available') || missionAsset(u.role) || u.mission==='WAITING FOR TRANSPORT') continue
    if (isAir(u.role)) {
      if (u.fuel <= 0 || (u.serviceStatus === 'INSUFFICIENT MISSION FUEL RESERVE' && (u.altitude || 0) <= .5)) continue
      const target=u.path[0]||state.objectives[Math.floor(state.objectives.length / 2)];if(u.mission!=='WITHDRAW')u.mission=u.role==='RECON_UAV'?'RECON':'CAS';const angle=Math.atan2(target.x-u.x,target.y-u.y),delta=Math.atan2(Math.sin(angle-u.heading),Math.cos(angle-u.heading));u.heading+=Math.max(-.035,Math.min(.035,delta));
...
function applyGeometry(packet: GeometryPacket) {
  ...
  for(const u of state.units){if(isNaval(u.role)||isAir(u.role)||u.carrier||u.hp<=0||!nav.covered(u))continue;Object.assign(u,nav.nearest(u));for(const s of u.soldiers||[])if(s.status==='active'&&nav.covered(s))Object.assign(s,nav.nearest(s))}
}
