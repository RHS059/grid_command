...
export function updateSoldiers(state:BattleState,v:Visibility,nav:Navigation,nextId:()=>number,perception?:Perception){
  ...
  for(const u of state.units){if(isVehicle(u.role)||u.carrier||u.surrendered||u.mission==='BOARDING')continue;u.suppression=Math.max(0,(u.suppression||0)-.003)
    ...
    for(const [i,s] of (u.soldiers||[]).entries()){
      ...
      if(enemy&&(u.suppression||0)>.3){const reserved=u.tacticalIntent?.action==='COVER'&&u.tacticalIntent.destination?formation(u,i,u.tacticalIntent.destination):undefined;if(reserved)s.cover=reserved;else if(!s.cover&&state.tick%10===i%10){const p=v.cover(s,enemy);if(p&&nav.clear(s,p))s.cover=p}if(s.cover){move(s,s.cover,nav,2.5);s.action=dist(s,s.cover)>1?'cover':'peek';s.stance='crouch'}else{s.stance=(u.suppression||0)>.65?'prone':'crouch';s.action='idle'}s.aim=Math.atan2(enemy.x-s.x,enemy.y-s.y)
      ...
      }else{s.cover=undefined;s.stance='stand';const target=formation(u,i);if(dist(s,target)>1){move(s,target,nav,5);s.action='walk'}else{s.action=state.time-s.shotAt<.2?'fire':'idle'}s.aim=enemy?Math.atan2(enemy.x-s.x,enemy.y-s.y):s.heading}
    }
