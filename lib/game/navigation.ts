import { local, type GeometryPacket, type Point } from './types'
...
export class Navigation {
  ...
  covered(p: Point) { if(!this.strict)return true;const g=(this.sectors.get(sectorKey(p))||this.sectors.get('legacy'))?.terrain;return !!g&&p.x>=g.x&&p.y>=g.y&&p.x<=g.x+(g.width-1)*g.step&&p.y<=g.y+(g.height-1)*g.step }
  ...
  import(features: GeometryPacket['features']) {
    for(const f of features){const id=`${f.key}@${this.importing?.sector||'legacy'}`;if(this.polygons.has(id))continue;this.polygons.add(id);const rings=f.rings.map(r=>r.map(local)),r=rings[0];if(!r?.length)continue
      ...
      for(let x=minX;x<=maxX;x++)for(let y=minY;y<=maxY;y++){const k=`${x},${y}`,p=point(k);if(this.strict&&!this.covered(p))continue;if(inside(p,r)&&!rings.slice(1).some(h=>inside(p,h)))this.mask.set(k,f.water?2:Math.max(1,this.mask.get(k)||0))}
    }
  }
  nearest(p: Point): Point {
    if(!this.covered(p)||!this.mask.has(key(p)))return {x:p.x,y:p.y}
    const cx=Math.floor(p.x/CELL),cy=Math.floor(p.y/CELL)
    for(let r=1;r<20;r++)for(let y=-r;y<=r;y++)for(let x=-r;x<=r;x++){if(Math.abs(x)!==r&&Math.abs(y)!==r)continue;const q=point(`${cx+x},${cy+y}`);if(this.covered(q)&&!this.mask.has(key(q)))return q}return {x:p.x,y:p.y}
  }
  clear(a:Point,b:Point){const n=Math.max(1,Math.ceil(Math.hypot(a.x-b.x,a.y-b.y)/8));for(let i=0;i<=n;i++){const p={x:a.x+(b.x-a.x)*i/n,y:a.y+(b.y-a.y)*i/n};if(!this.covered(p)||this.mask.has(key(p)))return false}return true}
  strategic(from:Point,to:Point):Point[]{
    ...
    const a=closest(from),b=closest(to),step=a<=b?1:-1,path:Point[]=[]
    if(a!==b)for(let i=a;i!==b+step;i+=step)path.push({...CORRIDOR[i]});path.push({x:to.x,y:to.y});return path
  }
