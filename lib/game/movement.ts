...
export function travel(u:Unit,to:Point,nav:Navigation,time:number,speed=CATALOG[u.role].speed,altitude=80){
  ...
  if(d<2){if(air&&Math.abs((u.altitude||0)-altitude)>.1){u.altitude=(u.altitude||0)+Math.sign(altitude-(u.altitude||0))*Math.min(1,Math.abs(altitude-(u.altitude||0)));return false}u.path=[];return true}
  ...
  u.travelStatus=undefined;u.heading=Math.atan2(next.x-u.x,next.y-u.y);u.x=p.x;u.y=p.y;if(step>=segment)u.path.shift()
  if(air)u.altitude=(u.altitude||0)+Math.max(-.6,Math.min(.6,altitude-(u.altitude||0)))
  return false
}
